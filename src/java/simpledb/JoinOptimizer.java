...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= joins.size(); i++) {
            // subset of joins, length = i
            Set<Set<LogicalJoinNode>> subset = enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> s : subset) {
                double bestCost = Double.MAX_VALUE;
                CostCard bestPlan = new CostCard();
                // for each subset, calculate remove join.
                // length: subset - 1
                for (LogicalJoinNode node : s) {
                    CostCard costCard = computeCostAndCardOfSubplan(stats, filterSelectivities, node, s, bestCost, pc);
                    if (costCard != null) {
        ...
        return pc.getOrder(enumerateSubsets(joins, joins.size()).iterator().next());
    }
